...
Like AM, each heuristic has a level of worth. Higher-worth heuristics are more likely to be invoked. Each heuristic has a `Creator`, so that if a heuristic rule rises in worth, its `Creator` also rises in worth. The original heuristic rules have `Creator` being Lenat himself.
...
Other than winning the sci-fi naval battle tournaments, Eurisko also worked on number theory, set theory, Traveler RPG, and MOS design. In MOS, it designed some new circuit elements that were fabricated, such as a more efficient flip-flop that was "difficult to produce masks for and difficult to fabricate, but extremely small and fast".

But we are really here to hear about the naval battles.

Lenat decided to enter the 1981 Trillion Credit Squadron tournament for the Traveller tabletop RPG game. The goal of the game is to build a fleet of spaceships with a budget of 1 trillion credits, and some other constraints, like ≤ 100 ships. Lenat coded the rules into Eurisko. Every evening Eurisko would run through its tasks, including Traveller games, MOS design, math problems, and so on. Every morning Lenat would check on the last night's results, remove some, and add some. Manual intervention was necessary since otherwise Eurisko can be stuck with bad heuristics for a long time, and because of weird meta-bugs. Lenat estimated that the final Eurisko had accumulated 1300 CPU-hours of runtime in total on a Xerox 1100 Lisp machine, and the Traveller win was "60/40% Lenat/Eurisko".

Some heuristics that Lenat programmed in at the start were
...
Other than hard-coded heuristics, Eurisko discovered many heuristics on its own (of course), such as the "nearly extreme" rule: In almost all Traveller TCS fleet design situations, the right decision is go for a nearly--but not quite--extreme solution. 
...
Fortunately for Eurisko and Lenat, this problem was mostly a matter of nonlinear programming, since the actual battles were "tactically trivial". Also importantly, the problem was hard enough, and nonlinear enough, for Eurisko to show its edge over linear programming and manual designs.
...
At the 1981 championship, Eurisko's fleet consisted of 75 "Eurisko class" ships (very slow, very armored, and many small missiles) discovered thanks to the "nearly extreme" heuristic, a tiny boat that is almost impossible to hit ("stalemate guarantor"), and a ship designed to hit the tiny boat, and a few others. Whereas most battles took 2--4 hours, Eurisko's opponents resigned in a few minutes, because typical opponents had around 20 large ships against Eurisko's 96 ships. Each round would destroy Eurisko's 15 ships and 5 of opponent's ships. One round was enough for the opponent to realize this, and resign.

> Its second opponent did some calculations and resigned without ever firing a shot. The subsequent opponents resigned during their first or second round of combat with this fleet.
...
The first plan, proposed in 1985, had 3 phases [@lenatCycUsingCommon1985]:
...
Most articles had just 1 paragraph, and 1 paragraph took about 1 person-day to encode, they estimated that phases 1 and 2 would take 150 person-years to complete, so with a crew of 20, the plan seemed doable.
...
In 2016, Lenat finally declared the Cyc project "done" and set about commercializing it, both via Cycorp and via Lucid.ai, a company founded in 2015.
...
This timing of commercialization coincides suspiciously with the end of almost all projects except ResearchCyc. Even [the documentation](https://web.archive.org/web/20160825080811/http://www.cyc.com/documentation/) [went offline](https://web.archive.org/web/20170802104610/http://www.cyc.com/documentation/) in 2016 and could now only be accessed for commercially registered accounts! It looks like a total internal pivot as the company focused on commercialization and shut down all services that are not high priority for the bottom line.

After cyberstalking the Cyc for days, I have gathered basically every numerical data about Cyc over the years, and I combined them into a [spreadsheet that you can analyze yourself](code/Cycorp%20claims.xlsx). In short, Cyc has grown to 30M assertions over the years, and is still incomplete. Lenat's claim of "Phase 1 complete by 1995" was amended to around 2015, and he claimed it was basically complete by 2016. Still, Cyc is stuck at that stage, with no self-learning or AGI in sight.

![The progress of Cyc project over the period of 1989--2022. The number of assertions grew to 30M, the cost grew to \$200M, with 2000 person-years. Source: [my obsessive cyberstalking](code/Cycorp%20claims.xlsx).](figure/Cyc_progress_history.png)

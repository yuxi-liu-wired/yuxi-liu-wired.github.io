...
As a simpler analogy, we can construct the 3-dimensional real-space $\R^3$ as the 3-dimensional affine space $\mathbb A^3$ decorated with an origin point $O$, . Then, we can write $\R^3 := (\mathbb A^3, O)$, and then we might be tempted to ask "What is the affine-space-in-itself, before it received the origin point?". However, we can equally well start with $\R^3$, and then quotient out $T$, the group of translations, to obtain $\mathbb A^3 := \R^3 / T$.

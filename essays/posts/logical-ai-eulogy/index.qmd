...
The MYCIN caugh the eye of people outside of AI community. Here seemed like a system that might actually be commercialized and do the work of doctors. And not just doctors -- what else might it do?
...
It is hard to write this section. I tried it a few times, but it always end up being way too boring. So I will describe it briefly. Knowledge, in the context of expert systems, is basically object-oriented programming + relational database + first-order logic.

There, now I've said it. It explains why expert systems is so boring. The useful parts of it ended up being Java + SQL. 

To see this, consider a few examples of what counts as "knowledge" in expert systems.

The frame: Looks exactly like data types in Java. Multiple frames connected in a semantic graph: Looks exactly like a [UMD diagrams](https://en.wikipedia.org/wiki/Unified_Modeling_Language).

![A frame system. It looks exactly the same as those UMD diagram I hated back when I learned Java programming. [@russellArtificialIntelligenceModern1995, figure 10.7]](figure/frame_system.png)

Unification: This is finally something new. It is an operation in first-order logic. You can just generalize a single example:
...
There were mainly two kinds of "knowledge acquisition". Most of it was slow and painful, with knowledge engineers "eliciting" knowledge from experts by interviews and observations. It was a cottage industry for spinning out knowledge, and many recognized this as the great problem for scaling expert systems up.
...
From various reading, it seems to me that the largest possible expert systems assembled by such a cottage industry contained around 10,000 rules. Anything bigger than that basically was too complicated for its own good. Except the Cyc.
...
There were many attempts at automatic knowledge acquisition, starting with meta-DENDRAL, and culminating with Cyc. They all failed to scale up. Since we will describe the Cyc in detail, we will just briefly describe the meta-DENDRAL.
...
The meta-DENDRAL was developed near the end of DENDRAL, when the number of rules became hard to manage. The team decided to see if they could design an expert system that is an expert at learning from spectroscopic data -- not *generic* learning from *generic* data. Their hypothesis was "knowledge acquisition is itself a knowledge-based task". To do this, they 

In 1976, they reported some new rules discovered by meta-DENDRAL for classifying [androstanes](https://en.wikipedia.org/wiki/Androstane).
...
My guess is that the expert system boom took place because there was something genuinely new: In 1980, laypeople outside the computer community knew very well that computers can crunch statistical datasets and numerical simulations, and that they can perform if-then-else logical control flows, but they thought that's *all* they could do. In the 1970s, expert systems with uncertainty quantification was something new in the research level, as barely refined ore. Around 1980, it had been roughly packaged up as EMYCIN by researchers and technologists, who had used EMYCIN to solve real problems. So, during 1980s, expert systems reached the last stage: to be hammered into a tool for business.
...
Many years later, surrounded by the humming servers of the Cyc database, Douglas Lenat was to remember that distant afternoon when he first conceived of encoding all of common sense.
...
The `PLANNER`, just like the GPS of Simon and Newell, starts with the goal as the root node, and searches for a chain of logical operations that would eventually lead to a leaf node equal to the current state of the world.[^shrdlu-logic] This was called "backward chaining". It then stores this chain of logical operations, and converts it into a sequence of `MOVER` commands and sends them to `MOVER`. When asked "Why did you do it?", it would convert this plan into an explanation in English.
...
### Collapse of computer vision

Unfortunately, logical AI approach to computer vision simply could not handle any realm slightly more complex than geometric perfection.
...
Still, since Shakey's computer ran at just 0.3 MIPS, it was doing its best. As compute improves, the best paradigm changes from the logical approach, to the statistical, to the neural.
...
TODO quote Duda Hart, compare Guzman, 

Moravec reports an early 1977 robot that featurizes camera images taken between two steps in the robot trajectory and compares them to triangulate the scene. It only worked on very clean and uncluttered scenes with sharp images:
...
Lest some, out of moral integrity, claim that mixing military and AI only hurts everyone, there was one definite success story: DART.

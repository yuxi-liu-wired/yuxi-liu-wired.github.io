...
format:
  html:
    ...
    resources:
        - "figure/**"
description: "How to think like a classical thermodynamic-economist, delivered with many illustrations and some sci-fi metaphors. Particular emphasis on what traditional pedagogy gets wrong. Prerequisite: multivariate calculus and mathematical maturity."
...
Unlike analytical mechanics, which is typically taught to students intent on reaching the abstract plane of modern theoretical physics, and is thus deep but not wide, a typical course on classical thermodynamics is shallow, but very wide, and taught to a wide student base -- theoretical physicists, thermal engineers, electric engineers, chemists, biologists... When something must be taught to a wide audience, and is both wide and deep, depth is sacrificed first. This is perfectly practicable, but it leaves a small minority confused with a sinister feeling that the teachers has abused their trust in them. I am in that minority.
...
The prerequisites are multivariate calculus and mathematical maturity.
...
[^anecdote-square]: Some children are confused when they heard that squares are rectangles too. I hope you won't be equally confused when you heard that mechanical systems are also thermodynamic systems.
...
Everything about a thermodynamical system is known once we specify how its entropy is a function of its macroscopic properties. 
...
Based on the following diagram, prove that if we have three scalar functions $x, y, z$ on a smooth 2D surface, then $\left(\frac{\partial x}{\partial z}\right)_y\left(\frac{\partial y}{\partial x}\right)_z\left(\frac{\partial z}{\partial y}\right)_x = -1$. Generalize this to higher dimensions.
...
[^equally-conserved]:
    The first law of thermodynamics had always struck me as oddly out of place, almost like a joke that I could not catch, like 
...
::: {#thm-envelop-helmholtz}

## envelop theorem for Helmholtz

For any inverse temperature $\beta > 0$ and thermodynamic properties $X$, let $U^*$ be the optimal internal energy that maximizes Helmholtz free entropy, then
...
Economically speaking, the second equation is a special case of the [envelop theorem](https://en.wikipedia.org/wiki/Envelope_theorem), just like Hotelling's lemma.
...
![When there is a kink in the curve of $S(U, X)$, the system has indeterminate temperature.](figure/indeterminate_temperature_SU_curve.png)
...
![When there is a double tangent in the curve of $S(U, X)$, the system has a first-order phase transition with latent entropy and energy.](figure/first_order_phase_transition_SU_curve.png)
...
The difficult thing about classical thermodynamics is that there are so many quantities (temperature, volume, pressure...). The saving grace is that it turns out that there are only a few degrees of freedom.[^parthian-shot]
...
Now, by the [area ratio construction](#thm-area-ratio), there exists two functions $f_T, f_S$, such that the new coordinates
...
The pH value of a solution is not $pH = -\log_{10} [H^+]$, which has the wrong units. It is not even $pH = -\log_{10} \frac{[H^+]}{[H^+]^\circ}$, since the $H^+$ particles might not behave like an ideal gas. The actually correct definition is [@mccartyPHParadoxesDemonstrating2006]
...
For real gases and real solutions, the chemical activity might deviate significantly from the above approximation. In this case, we typically have no recourse except by checking a table of chemical thermodynamics. They typically don't directly write down the chemical activities, but [*fugacity coefficients*](https://en.wikipedia.org/wiki/Fugacity). There is nothing particularly deep about fugacity -- it is basically about rescaling the numbers to make the tables easier to make.
...
Differentiating, we find that the marginal value of each asset is equal in both subsidiaries:
...
The most common breakdown of extensivity is in gravitational systems. If we pute two astronomically-large boxes of "planet-particle gas" side-by-side, then their sum-total entropy is not equal to the sum of entropy of each box in isolation.
...
Marx, for all his interest in changing the world instead of describing it, did attempt to mathematically model aspects of a capitalist economy, though it is only of historical interest now. Samuelson wrote several papers trying to update Marx's theory into modern mathematical language, and thought of Marx the mathematical economist as a "minor post-Ricardian". [@bronfenbrennerSamuelsonMarxTheir1973]
...
And just like when I rediscovered Wigner rotation, as soon as I have figured out everything for myself, I knew the right words to search, and found that, of course, someone else has written this before [@smithClassicalThermodynamicsEconomic2008], repeatedly [@candealUtilityEntropy2001]. So why spend all this time to write another one? I think I have written this pedagogically. I don't care if it is not new, or that it has been said before with more symbols and theorems. I have a thing to say, so I will say it well.

...
From the hindsight of deep learning, the first hypothesis has been amply justified, the second refuted, and the third appears to be differ case-by-case.[^competence-threshold] Lenat, however, bet everything on the three assumptions, in a heroic 30-year-long journey to build Cyc, an expert system AGI.
...
There were many attempts at automatic knowledge acquisition, starting with Meta-DENDRAL, and culminating with Cyc. They all failed to scale up.
...
> 
> Both Monte Zweben, President of Red Pepper Software (San Mateo, CA), and Tom Laffey, Chief Technology Officer of Talarian (Mountain View, CA), had similar stories to tell regarding the successful launch of companies selling real-time expert system tools: never, ever call the products "AI". "Don’t mention AI if you want any venture capital money," Laffey warned. "Call it something else, such as advanced decision systems. These days, people come to AI almost as a last resort." Zweben’s company shared the same fate as Talarian: he had to remove or restate every mention of AI from the Red Pepper prospectus before investors would take him seriously. Despite the literally thousands of successfully deployed intelligent applications, AI’s reputation continues to suffer because nobody is giving credit to the AI component. Zweben suggested wishfully that AI companies insist on being recognized for their contribution to applications, perhaps even demanding an ‘AI inside’ label, it la Intel’s ‘Intel inside’ label on PCs.
> 
...
In hindsight, there were premonitions of the winter even in an early report on XCON, the system that saved \$40m/year. According to the report [@bachantR1RevisitedFour1984], XCON was originally supposed to be an expert in configuring [VAX](https://en.wikipedia.org/wiki/VAX) and [PDP-11](https://en.wikipedia.org/wiki/PDP-11) machines, and the knowledge base coded everything in a bespoke format. In 1982, DEC wanted to use XCON for configuring other machines using a common database. This apparently minor change in format required an almost complete and nontrivial rewrite.

> The component descriptions had been developed exclusively for R1 and were tailored to the configuration task. As Digital developed other knowledge-based systems for other purposes, it became desirable to have a common data base, where the components were categorized in a less ad hoc fashion. Before Rl could use the new descriptions, nearly all of its rules (about 2000 at the time) had to be changed, and for several hundred of these rules, the task of reformulation took considerable thought.

It was difficult for people to write rules for XCON. Unlike what they expected, expert systems could not be written by the experts. The knowledge engineers, attuned to the ontological pitfalls, were needed. Further, the more rules XCON already had, the harder it was. A form of diminishing returns bit. It is no wonder why XCON eventually stagnated at the maintainability limit of 16,000 rules.

> It was assumed initially that the component descriptions could be created by people who knew a lot about the components, but knew little about how R1 would use the descriptions. As it turned out, creating useful descriptions is not all that straightforward. It often is not clear what "configuration level" means, not clear what attributes are required, and not clear what knowledge to put in the rules and what in the data base. In order to know what information a description should contain, it is necessary to know how the information is going to be used. In order to know how the information is going to be used, it is necessary to know something about the component. 
> 
> ... now that R1 has grown substantially, its sheer magnitude seems to serve as a barrier to the would-be developer. It takes much longer now for someone who joins the group to gain an adequate understanding of how Rl does configuration.
...
... that's one worthy of a [Coding Horrors](https://blog.codinghorror.com/) post, with the title "Real Programmers can write FORTRAN in any language".
...
In 1984, spreadsheet and object-oriented programming was so glamorous they deserved full-page pictures. Now they suffer under the ignominious boredom that is Microsoft Excel and Java. [@kayComputerSoftware1984]
...
... but for [Cyc](https://yuxi-liu-wired.github.io/essays/posts/cyc/).

### TODO insert this somewhere

There is a wide-sense and a narrow-sense expert systems, getting confused in the AI spring.

The narrow-sense expert system is the original sense. According to it, an expert system is made of logical inference engines running over a large and extensible knowledge base of rules, written in a formal logical language. "Programming" an expert system is mainly in adding rules to the base, not in changing the engines. The system in general improves with more rules, although it is not guaranteed.

The wide-sense expert system involves spreadsheets, object-oriented programming, and all the other software innovations that came in the 1980s.

Most of the promises of narrow-sense expert systems failed for the given reasons. They were a powerful tool in search of a problem to solve. In practice, most projects turned out to be too easy for expert systems, in which case standard if-then-else over a standard database eventually replaced expert systems. This had the advantage that computer programmers were much cheaper and easier to train than knowledge engineers, and these programs were cheaper to build and maintain than expert systems. Some problems, such as the projects in speech recognition and self-driving (TODO cite SCI) we still recognize as genuine AI projects, turned out to be too hard for expert systems, in which case they suffered dibilitating scaling issues. There just turned out to be few problems for which expert systems were not too powerful, not too powerless, but just right. Such things are hard to predict.

How did the fever start? A coincidence of the two waves coming out of the labs created the fever. To an outsider, everything seemed rather magical. Expert systems were magic. Automatic spreadsheets were magic. Object-oriented programming was magic. Microcomputers were magic. In the exciting confusion, people bought the whole package, wide-sense and narrow-sense. When the reckoning came, the wide-sense survived well, but changed their names. The narrow-sense found itself without a product-market fit, and mostly died out.

...
This section based on [Wikipedia](https://en.wikipedia.org/wiki/Logistic_map).
...
The Ising model on $\Z^2$ is not a single theory, but an entire infinite-dimensional space of possible theories. Each Ising model . 
...
| d | n | Theoretical Model | Physical Systesm | Order Parameter |
|---|---|---|---|---| 
...
$$
\ub{\int\cdots\int}{$N \to \infty$, with constraint $C$} (\text{something}) \rho(x)d^N x
$$
...
4. The inner integral splits into a product of $N$ terms, then evaluate each term, resulting in something like

$$
=_{\ln} \ub{\int\cdots\int}{$n$} (\text{something else}) e^{NS[\lambda]} d^n\lambda
$$

where $S$ is sometimes called the **field free energy**.
...
We employ field-theoretic techniques to compute the rate function, treating both the empirical distribution $\hat{p}$ and the true distribution $p$ as fields over a finite set of points:
...
If only I understood what this is saying, then I could write this section. [@mehtaExactMappingVariational2014]

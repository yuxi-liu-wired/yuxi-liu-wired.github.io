...
Each state $x\in \Omega$ is a text: a stream of letters infinite both forwards and backwards. The state space $\Omega$ is the all possible texts $(X_n)_n$. We assume that all letters come from the same finite-size alphabet, for example, the 128 ASCII symbols. 

The map on the state space $T : \Omega \to \Omega$ is the shift map, defined by

$$
T(\dots, X_{-1}, X_0, X_1, \dots) := (\dots, X_0, X_1, X_2, \dots)
$$

Repeatedly applying the shift map $T$ is just reading the stream, one letter at a time:

$$
\text{...Lorem ipsum ...} \mapsto \text{...orem ipsum d...} \mapsto \text{...rem ipsum do...} \mapsto \cdots
$$

A periodic point of $T$ is a text that repeats itself. For example, $X := \text{... and and and ...}$ satisfies $T^4X = X$.

A $T$-invariant set $S\subset \Omega$ is a set of texts, such that if we take any text $X$ from $S$, and jump either forwards or backwards for an arbitrary amount, we get another set in $S$. In other words, $S$ is a set of letter streams where there is no origin: you can start reading from any letter.
...
To be ergodic means that there is no such partition. In fact, we have the theorem: 

If the state space has a countable basis, and any nonempty open set has positive measure, then almost any $x\in\Omega$ has dense orbit.

Proof: Let $U$ be a nonempty open set. Since $\Omega - \cup T^{-i}U$ is $T$-invariant and has non-full measure, it has zero measure. Therefore $\cup(\Omega - \cup T^{-i}U)$, the set of all points with non-dense orbit, has zero measure.

That is, if we randomly sample two pieces of text, and we shift one piece of text for long enough, we can find longer and longer stretches of text that matches the second text. It is like the old story about how you can find the entirety of Shakespeare in the digits of $\pi$, if you go far enough into the digits.

...
Now we define the entire perceptron machine by $\sum_{m \leq n} M_{n - m} x_m x_n (f_{m, n}(x) - 1/2)$. If the sequence of $M_0, M_1, ...$ grows fast enough, then the largest bracket $(m, n)$ would swamp every smaller bracket contained within, and so the entire perceptron machine is effectively first testing for the smallest interval containing the figure, before testing the symmetry within that interval.
...
Chapter 9 extends the stratification idea of chapter 7 to an arbitrary level.  [TODO]

Chapter 10 is essentially made by "finitizing" the construction of chapter 7. Whereas chapter 7 concerns machines operating over infinite planes, chapter 10 concerns sequences of machines operating over a sequence of squares growing towards infinity. For example, instead of testing for reflection symmetries on $\Z$, it considers testing for reflection symmetries on $\{-n, -n+1, \cdots, n-1, n\}$ for $n \to \infty$. Whereas in the construction @exm-line-symmetry, we saw coefficients growing exponentially, chapter 10 shows that this phenomenon of exponentially growing coefficients is common.

A single example illustrates the general point.
...
Section 10.2 and 10.3 constructs two pathological examples. By restricting the shapes of hidden perceptron masks, they proved that certain predicates required superexponential weights. Section 10.4 purports to extend the group invariance theorem to the infinite case.[^theorem-10-4-1] None of these are used elsewhere in the book. They are virtuoso performances.

[^theorem-10-4-1]: They purported in Theorem 10.4.1 that, if the weights are bounded, then the group invariance theorem applies again, but the proof is false, with a counterexample $\sum_{n \in \Z}e^{-n^2}x_n - e^{-n^4}x_nx_{n+1}$. The theorem *might* still be correct, but I cannot prove it.


### Perceptron learning theory

In Chapter 11, they revisited the computational reduction, and suggested that real-valued perceptrons are not better than binary perceptrons, because ...

::: {#def-perceptron-learning}
## Perceptron learning

To train a perceptron machine is to *fix* its hidden perceptrons, and adjust the weights and threshold of *only* the single output perceptron.
:::

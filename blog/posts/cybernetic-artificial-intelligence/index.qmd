...
[^no-details-orthogonal-polynomial]: Of course, this is not the most natural or productive way to show this, but this post is not about orthogonal polynomial theory. So the proof is meant to only build trust, not general competency in orthogonal polynomial theory.
...
$$\int_\R e^{-x^2}H_n(x) H_m(x) dx = \sqrt\pi 2^n  n! \delta_{mn}$$
:::
...
Let $x(t)$ be a white noise process with variance $1/2$, and let $c_0(t), c_1(t), ...$ be its Laguerre coefficients, then:

* the joint stochastic process $(c_0(t), c_1(t), ...)$ is stationary;
* for any fixed $t\in \R$, the random variables $c_0(t), c_1(t), ...$ are independent samples of the standard gaussian distribution $\mathcal N(0, 1/2)$.
:::
